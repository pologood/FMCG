...
[#--活动-团购--]
    <div id="act-0" class="list_item ACT828">
        <div class="top">
            <img src="${base}/resources/app/3.0/image/activity_924_titler_001.png" width="100%"/>
                    ...
                    [#list adPosition as ads]
                        ...
                        [#if ads.linkId??&&ads.linkId?has_content]
                            [#if ads.linkType=="product"]
                                [#assign url='rzico://product?id=${ads.linkId}' ]
                            [#elseif ads.linkType=="tenant"]
                                [#assign url='rzico://tenant?id=${ads.linkId}' ]
                            [/#if]
                        [#else ]
                            [#assign url=ads.url]
                        [/#if]
                        [#if ads.linkType=="community"]
                            [#assign url='rzico://community?id=${ads.url}' ]
                        [/#if]
                        <div class="item">
                            <a  href="${url}">
                                <div class="img">
        ...
        [@adActivity id = activity1 areaId=area.id count=20]
            [#if adPosition?has_content]
                [#list adPosition as ads]
                    ...
                    [#if ads.linkId??&&ads.linkId?has_content]
                        [#if ads.linkType=="product"]
                            [#assign url1='rzico://product?id=${ads.linkId}' ]
                        [#elseif ads.linkType=="tenant"]
                            [#assign url1='rzico://tenant?id=${ads.linkId}' ]
                        [/#if]
                    [#else ]
                        [#assign url1=ads.url ]
                    [/#if]
                    [#if ads.linkType=="community"]
                        [#assign url1='rzico://community?id=${ads.url}' ]
                    [/#if]
        ...
        <div class="items">
            ...
            [#list activityPlanning.tenants as tenant]
                [#list tenant.tags as tag]
                    [#if tag.id==32]
                        <li>
                            <a href="rzico://tenant?id=${tenant.id}">
                                <div class="img">
...
<div id="actGotop" class="actGotop ACT828">
    <img src="${base}/resources/app/3.0/image/act_Top_002.png" width="100%"/>
</div>
<script src="${base}/resources/app/3.0/js/handlebars.min.js"></script>
...
<script type="text/javascript" src="${base}/resources/app/3.0/js/plugin/lazyload/lazyload.min.js"></script>
<script type="text/javascript" src="${base}/resources/app/3.0/js/plugin/activity/count_down.js"></script>
<script type="text/javascript" src="${base}/resources/app/3.0/js/plugin/activity/scroller_tag.js"></script>
<script type="text/javascript" src="${base}/resources/app/3.0/js/plugin/activity/activity_828.js"></script>
<!-- JS:receiver-->
<script type="text/javascript">
    ...
    $(".lazy").picLazyLoad({
        threshold: 100,
        placeholder: _TH_.base + '/resources/app/3.0/images/AccountBitmap-product.png'
    });
    /**
     *倒计时调用
     */
    ...
    $(function () {
        ...
        [#--$.ajax({--]
                    [#--url: _TH_.base + "/app/activity/get/location.jhtml",--]
                    [#--type: "POST",--]
                    [#--data: {--]
        ...
        [#--).done(function (data) {--]
                    [#--if (data.message.type == "success") {--]
                        ...
                        [#--$(".lazy").picLazyLoad({--]
                            [#--threshold: 100,--]
                            [#--placeholder: _TH_.base + '/resources/app/2.0/images/AccountBitmap-product.png'--]
                        [#--});--]
                    [#--} else {--]
                        [#--$(".ONCET").tip("addTask", {--]
