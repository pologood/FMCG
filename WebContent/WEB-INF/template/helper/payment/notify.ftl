...
<link href="${base}/resources/helper/css/payment.css" rel="stylesheet" type="text/css" />
  ...
  [#include "/helper/include/header.ftl" /]
	...
	<div class="payment">
			...
			<div class="bottom" style="border-top: solid 1px #e5e5ce;">
				[#if payment.type == "payment"]
					<a href="${base}/helper/member/order/view.jhtml?sn=${payment.order.sn}">${message("shop.payment.viewOrder")}</a>
				[#elseif payment.type == "recharge"]
					<a href="${base}/helper/member/deposit/list.jhtml">${message("shop.payment.deposit")}</a>
				[/#if]
				| <a href="${base}/helper">${message("shop.payment.index")}</a>
			</div>
	</div>
	</div>
