        ...
        <div class="wrapper" id="wrapper">
            ...
            <div class="page-nav page-nav-app vip-guide-nav" id="app_head_nav">
                <div class="js-app-header title-wrap" id="app_0000000844">
                    ...
                    <dl class="app-info">
                        <dt class="app-title" id="app_name">优惠订单</dt>
                        <dd class="app-status" id="app_add_status">
                        </dd>
                        <dd class="app-intro" id="app_desc">查看使用我店优惠券的订单。</dd>
                    </dl>
                </div>
            </div>
            <form id="listForm" action="favorable_list.jhtml" method="get">
                <input type="hidden" name="status" value="${status}" id="status_val">
                <div class="bar">
                    <div class="buttonWrap">
                        <a href="${base}/helper/member/trade/favorable_list.jhtml" class="iconButton">
                       ...
                       placeholder="结束时间" style="width:150px;" /> -->
                        <input type="text" id="keyword" name="keyword" value="${keywords}" maxlength="200"
                ...
                <div class="list">
                    <table id="listTable" class="list">
                        ...
                        <tr>
                            ...
                            <td>
                                [#if trades.tenantCouponCode??]
                                    ${trades.tenantCouponCode.coupon.name}
                                [/#if]
                            </td>
                            <td>
                                [#if trades.tenantCouponCode??]
                                    ${trades.tenantCouponCode.coupon.amount}
                                [/#if]
                            </td>
                            <td>
                                [#if trades.tenantCouponCode??]
                                    [#if trades.tenantCouponCode.coupon.status=="canUse"]
                                    可领用
                                    [#elseif trades.tenantCouponCode.coupon.status=="unBegin"]
                                    为开始
                                    [#elseif trades.tenantCouponCode.coupon.status=="unUsed"]
                                    已领完
                                    [#elseif trades.tenantCouponCode.coupon.status=="Expired"]
                                    已过期
                                    [/#if]
                                [/#if]
                            </td>
                            <td>
                                [#if trades.tenantCouponCode??]
                                    [#if trades.tenantCouponCode.isUsed=="true"]
                                    已使用
                                    [#else]
                                    未使用
                                    [/#if]
                            ...
                            <td>
                                [#if trades.tenantCouponCode??]
                                    ${trades.tenantCouponCode.member.username}
                                [/#if]
                            </td>
                        </tr>
                        [/#list]
