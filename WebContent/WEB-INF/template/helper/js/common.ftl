			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserUploadButton = $('<a href="javascript:;" class="browserUploadButton button">' + message("helper.browser.upload") + '<\/a>').appendTo($browserForm);
...
$(function(){
	//未开启店铺状态，关闭菜单功能
	var url=$("#baseUrl").val()+'/helper/member/tenant/checkTenant.jhtml';
	$.ajax({
		url:url,
		type:'post',
		dataType:'json',
		success:function(data){
			if(data.type=='error'){
				$(".am-accordion-bd a").each(function(){
					$(this).attr("href","javascript:;");
				});
				$(".am-accordion-bd a").click(function(){
