...
<link href="${base}/resources/helper/css/detail.css" type="text/css" rel="stylesheet" />
<script type="text/javascript" src="${base}/resources/helper/js/jquery.js"></script>
<script type="text/javascript" src="${base}/resources/helper/js/jquery.jqzoom.js"></script>
<script type="text/javascript" src="${base}/resources/helper/js/jquery.tools.js"></script>
...
$().ready(function() {
	...
	$specificationValue.click(function() {
		...
		$.each(productMap, function(i, product) {
			if (product.specificationValues.length == selectedIds.length && contains(product.specificationValues, selectedIds)) {
				if (product.id != null) {
					location.href = "${base}/helper/product/content/" + product.id+".jhtml";
					locked = false;
				}
				return false;
			}
	...
	$addCart.click(function() {
		...
		if (/^\d*[1-9]\d*$/.test(quantity) && parseInt(quantity) > 0) {
			$.ajax({
				url: "${base}/helper/cart/add.jhtml",
				type: "POST",
		...
		if (/^\d*[1-9]\d*$/.test(quantity) && parseInt(quantity) > 0) {
			$.ajax({
				...
				success: function(message) {
					...
					{
						$.message(message);
						location.href = "${base}/helper/cart/list.jhtml";
					}else{
						$.message(message);
					}
				}
	...
	$addFavorite.click(function() {
		$.ajax({
			url: "${base}/helper/member/favorite/add.jhtml?id=${product.id}",
			type: "POST",
			dataType: "json",
			cache: false,
			success: function(message) {
	...
	$.ajax({
		url: "${base}/helper/product/history.jhtml",
		type: "GET",
	...
	// 点击数
	$.ajax({
		url: "${base}/helper/product/hits/${product.id}.jhtml",
		type: "GET"
	});
	[#if setting.isReviewEnabled && setting.reviewAuthority != "anyone"]
			...
			$addReview.click(function() {
				if ($.checkLogin()) {
					...
					$.redirectLogin("${base}/helper/review/add/${product.id}.jhtml", "${message("shop.product.addReviewNotAllowed")}");
					return false;
				}
			});
	[/#if]
	...
	[#if setting.isConsultationEnabled && setting.consultationAuthority != "anyone"]
		...
		$addConsultation.click(function() {
			if ($.checkLogin()) {
				...
				$.redirectLogin("${base}/helper/consultation/add/${product.id}.jhtml", "${message("shop.product.addConsultationNotAllowed")}");
				return false;
			}
		});
	[/#if]
		...
		<!-- <div class="s_detail_top">
			...
			[@product_category_parent_list productCategoryId = productCategory.id]
				[#list productCategories as productCategory]
					<a href="${base}/helper/product/list/${productCategory.id}.jhtml">${productCategory.name}</a>>
				[/#list]
			[/@product_category_parent_list]
			<a href="${base}/helper/product/list/${productCategory.id}.jhtml" class="lastcolor">${productCategory.name}</a>
		</div> -->
			...
			<div class="detailcl">
				<div class="hotProduct w_js">
					...
					<div class="w_content" style="height:auto">
						...
						<p><a href="${base}/helper/${product.tenant.id}/index.jhtml">${product.tenant.shortName}</a></p>
						[#if product.tenant.qq??]
						<p class="w_lineme"><a href="tencent://message/?uin=${product.tenant.qq}&Site=在线QQ&Menu=yes"> <img src="${base}/resources/helper/images/v2.0/qq.gif">和我联系</a></p>
						[/#if]
						...
						<div class="w_goshop w_detailgoshop"><a href="${base}/helper/${product.tenant.id}/index.jhtml">进入商铺</a></div>
					</div>
				</div>
				<div class="h10"></div>
							...
							[@product_list tenantId=product.tenant.id count = 6 orderBy="monthSales desc"]
								[#list products as product]
									<li[#if !product_has_next] class="last"[/#if]>
										<a href="${base}/helper/product/content/${product.id}.jhtml" title="${product.name}">
						...
						<div name="review" class="detailcont s_dt3">
							...
							<div class="p_evaluate">
								...
								<div class="detail_pj">
									...
									<!-- <div class="handle">
										<a href="${base}/helper/review/add/${product.id}.jhtml" id="addReview">${message("shop.product.addReview")}</a>
									</div> -->
									</div>
									[@review_list productId = product.id count = 5]
										[#if reviews?has_content]
											...
											<p>
												<a href="${base}/helper/review/content/${product.id}.jhtml">[${message("shop.product.viewReview")}]</a>
											</p>
										[/#if]
									[/@review_list]
								[#else]
									<p>
										${message("shop.product.noReview")} <a href="${base}/helper/review/add/${product.id}.jhtml" id="addReview">[${message("shop.product.addReview")}]</a>
									</p>
								[/#if]
							</div>
						</div>
							...
							<div class="p_consulting">
								[@consultation_list productId = product.id count = product.consultations.length]
									[#if consultations?has_content]
										...
										<p>
											<a href="${base}/helper/consultation/add/${product.id}.jhtml" id="addConsultation" class="p_consultingbtn">[${message("shop.product.addConsultation")}]</a>
											<a href="${base}/helper/consultation/content/${product.id}.jhtml">[${message("shop.product.viewConsultation")}]</a>
										</p>
									[#else]
										<p>
											${message("shop.product.noConsultation")} <a href="${base}/helper/consultation/add/${product.id}.jhtml" id="addConsultation">[${message("shop.product.addConsultation")}]</a>
										</p>
									[/#if]
								[/@consultation_list]
							</div>
