...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
        ...
        <input type="hidden" name="activityPlanningId" value="${(adPosition.activityPlanning.id)!}" />
		<table class="input">
			<tr>
				<th>
			...
			<tr>
				...
				<td>
					<select  name="type">
						[#list types as type]
							<option [#if type==adPosition.type]selected="selected" [/#if] value="${type}">${message("AdPosition.Type." + type)}</option>
						[/#list]
					</select>
				</td>
			</tr>
				...
				<td>
					<select  name="productChannelId">
						...
						[#list productChannels as productChannel]
							<option value="${productChannel.id}" [#if productChannel == adPosition.productChannel] selected="selected"[/#if]>
								${productChannel.name}
							</option>
						[/#list]
					</select>
