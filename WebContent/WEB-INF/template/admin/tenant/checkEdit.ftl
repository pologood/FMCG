				...
				<tr>
					...
					<td>
						<select name="status">
							...
							<option value="wait" [#if ("wait" == (tenant.status)!)] selected[/#if] >审核中</option>
							<option value="success" [#if ("success" == (tenant.status)!)] selected[/#if] >已认证</option>
							<option value="fail" [#if ("fail" == (tenant.status)!)] selected[/#if] >已驳回</option>
						</select>
					</td>
				</tr>
			 	<tr>
					...
					<td>
						[#list tags as tag]
							<label>
								<input type="checkbox" name="tagIds" value="${tag.id}"[#if tenant.tags?seq_contains(tag)||tagSet?seq_contains(tag)] checked="checked"[/#if] />${tag.name}
							</label>
						[/#list]
					</td>
				</tr>
