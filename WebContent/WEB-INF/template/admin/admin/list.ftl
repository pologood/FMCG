	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as admin]
				<tr>
					<td>
						<input type="checkbox" name="ids" [#if admin.username=="admin"] title="admin" disabled="disabled" [#else] value="${admin.id}"[/#if] />
					</td>
					<td>
						${admin.username}
					</td>
					...
					<td>
						 [#if admin.username!="admin"]<a href="edit.jhtml?id=${admin.id}">[${message("admin.common.edit")}]</a>[/#if]
					</td>
				</tr>
			[/#list]
		</table>
