...
$().ready(function(){
	...
	//签收
	var $sign_order =$(".sign_order");
	$sign_order.live("tap",function(){
		var $this =$(this);
		$.ajax({
			url:"${base}/mobile/member/order/confirm.jhtml",
			data:{sn:$this.attr("sn")},
			dataType:"json",
			type:"post",
			beforeSend:function(){
				$sign_order.prop("disabled",true);
			},
			success:function(data){
				mtips(data.content);
				setTimeout(function(){location.href="${base}/mobile/member/trade/review.jhtml?sn="+$(this).attr("sn"); return false;},1500);
			},
			complete:function(){
				$sign_order.prop("disabled",false);
			}
		});
	});
	//跳转评价页面
	$(".reviewTrade").live("tap",function(){
			...
			success:function(list){
				if(list.length>0){
					for(var i=0;i<list.length;i++){
							...
							var tmpStr="";
							tmpStr+="<div class='order_num'><a href='javascript:;'>订单号:"+list[i].order.sn+"</a><p class='order_time'>"+Ordertime+"</p></div>" +
										"<div class='order_goods'> <a class='order_goods_img' href='javascript:;'>";
										 if(list[i].orderItems[0].thumbnail!=null){
											 tmpStr+="<img src='"+list[i].orderItems[0].thumbnail+"'/></a>";
										 }else{
											tmpStr+="<img src='' class='NoPicture'/></a>";
										 }
										tmpStr+="<a href='javascript:;' class='order_goods_a'>" +
											"<div class='order_goods_c'>" +
												"<p class='goods_name'>"+list[i].orderItems[0].fullName+"</p>" +
												"<p class='goods_style'>"+list[i].orderItems[0].product.specification_value+"</p>" +
											"</div>" +
											"<div class='order_goods_r'>" +
												"<p class='goods_price'>"+currency(list[i].orderItems[0].price,true)+"</p>" +
												"<p class='goods_amount'>x"+list[i].orderItems[0].quantity+"</p>" +
												"<p class='shop_name'>"+list[i].tenant.name+"</p>" +
											"</div>" +
										"</a>" +
										"</div>" +
										"<div class='goods_details'><a href='javascript:;' name='seeAll' sn='"+list[i].sn+"'>查看详情</a></div>" +
										"<div class='goods_others'>" +
										"<div class='goods_total'>" +
											"<p class='amount_title'>数量</p>" +
											"<p class='total_amount'>"+list[i].quantity+"</p>" +
											"<p class='total_title'>总计</p>" +
											"<p class='total_price'>"+currency(list[i].amount,true)+"</p>" +
										"</div>";
...
<section class="m_section">
  ...
  <div class="order_screen">
	    <ul>
		      ...
		      <li id="shipped" class="[#if type=='shipped']current_screen[/#if]"><img height="20px" src="${base}/resources/mobile/images/get.png" /><p>待收货</p>[#if waitSignCount>0]<span>${waitSignCount}</span>[/#if]</li>
		      <li id="unreview" class="[#if type=='unreview']current_screen[/#if]"><img height="20px" src="${base}/resources/mobile/images/appraise.png" /><p>待评价</p></li>
	    </ul>
  </div>
  <article class="order_list" id="m_scrooler_0" style="display:block">
    <div class="order_content m_bodycont_1">
      ...
      [#else]
	      <div id="pullUp">
				<span class="pullUpLabel">暂无待评价订单!.</span>
		  </div>
      [/#if]
    </div>
  </article>
