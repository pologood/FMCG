...
$().ready(function(){
	...
	$orderDetail.live('tap',function(){
		var $this = $(this);
		var sn = $this.attr("sn");
		location.href="${base}/mobile/member/order/tradeView.jhtml?sn="+sn;
		return false;
	});
...
<section class="m_section">
  ...
  <div class="order_screen">
	    <ul>
		      <li id="all" class="current_screen" ><img height="20px" src="${base}/resources/mobile/images/all-order.png" /><p>全部</p></li>
		      <li id="unpaid" class="[#if type=='unpaid']current_screen[/#if]"><img height="20px" src="${base}/resources/mobile/images/pay.png" /><p>待付款[#if waitPayCount>0]<span>${waitPayCount}</span>[/#if]</p></li>
		      <li id="unshipped" class="[#if type=='unshipped']current_screen[/#if]"><img height="20px" src="${base}/resources/mobile/images/send.png" /><p>待发货[#if waitShippingCount>0]<span>${waitShippingCount}</span>[/#if]</p></li>
  ...
  <article class="order_list" id="m_scrooler_0" style="display:block">
    <div class="order_content m_bodycont_1">
      ...
      [#else]
	      <div id="pullUp">
				<span class="pullUpLabel">您还没有下过订单!</span>
		  </div>
      [/#if]
    </div>
  </article>
