...
$().ready(function(){
	$("#productCategoryTenantId").find("a").on("tap",function(){
		location.href="${base}/mobile/product_channel/${productChannel.id}/index.jhtml?productCategoryId="+$(this).attr("productCategoryId"); return false;
	});
	$("#tenantList").find("a").on("tap",function(){
		location.href="${base}/mobile/delivery/"+$(this).attr("tenantId")+"/index.jhtml";return false;
	});
	$("#back_btn_t").on("tap",function(){
		location.href="${base}/mobile/index.jhtml";return false;
	});
});
// 2015-1-11 拖动加载
...
function pullUpAction () {
	setTimeout(function () {	// <-- 模拟网络拥塞，从生产中删除setTimeout的！
		...
		pageNum=pageNum+1;
		$.ajax({
			url:"${base}/mobile/product_channel/${productChannel.id}/loadmore.jhtml?productCategoryId=${productCategoryId}",
			type:"get",
			dataType:"json",
			...
			success:function(result){
				if(result.content.length>0){
					...
					$('#m_scrooler_all').iScroll('refresh');// 请记住，当刷新内容加载（即：在阿贾克斯完成
				}else{
					pullUpEl.querySelector('.pullUpLabel').innerHTML = '已经到底了...';
					return false;
				}
...
function loaded() {
	...
	myScroll = new iScroll('m_scrooler_all', {
		useTransition: true,
		onRefresh: function () {
...
<section class="m_section">
	<header class="m_header">
		<div class="m_headercont_1">
			<div class="m_return"><a id="back_btn_t" href="javascript:;" alt="返回"><div class="p_datag">返回</div></a></div>
			<div class="m_title" alt="选择日期">${productChannel.name}</div>
		</div>
	</header>
	<article class="m_article m_article_1" id="m_scrooler_all" >
			...
			<div class="m_shopt" id="m_shoptscroll" data--iscroll-="iscroll3" style="overflow: hidden;">
				<ul id="tenantList" style="width: 400px; -webkit-transition: -webkit-transform 0ms; transition: -webkit-transform 0ms; -webkit-transform-origin: 0px 0px; -webkit-transform: translate(0px, 0px) translateZ(0px);">
					[#list deliveris as delivery]
						<li><a tenantid="${delivery.id}" href="javascript:;"><img src="${delivery.tenant.thumbnail}" width="75px;" height="75px;"><br>
