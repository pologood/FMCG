	...
	//根分类li点击展开
	 ...
	 $root.on('tap',function(e){
		...
		$goDetail.on("tap",function(){
			...
			if($.checkLogin()){
				location.href = "${base}/mobile/groupon/content/"+promotionId+"/"+productId+".jhtml";
				return false;
			}else{
				mtips("参团请先登录!");
				setTimeout(function(){location.href = "vsstoo://login/?redirectURL=mobile/groupon/content/"+promotionId+"/"+productId+".jhtml";}, 1 * 1000);
				return false;
			}
		});
		...
		//加载更多
		 $more.on('tap',function(){
			 ...
			 $pageNumber.val(pageNumber);
			 $.ajax({
					url : "${base}/mobile/groupon/addMore.jhtml",
...
<section class="m_section">
	<header class="m_header">
		<div class="m_headercont_1">
			...
			<div class="m_title" alt="选择日期">团购列表</div>
	...
	<article class="m_article m_article_list" id="m_scrooler">
		<div class="m_bodycont">
			<form id="productForm" action="${base}/mobile/groupon/list.jhtml" method="get">
			...
			<div class="m_listcont m_shoplist m_tuanlist">
				<ul id="productList">
					...
					<li>
						<a href="${base}/mobile/groupon/content/${promotion.id}/${promotion.defaultProduct.id}.jhtml">
							<div class="m_productk">
			...
			[#else]
				<div class="m_more" ><span >暂无团购活动</span></div>
			[/#if]
