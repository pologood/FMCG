...
<head>
    ...
    <meta name="" content=""/>
    <title>${setting.siteName}-供应商-订单结算</title>
...
[#include "/b2c/include/supplier_top.ftl"]
    ...
    <form id="listForm" action="${base}/b2c/member/supplier/order_settle_account.jhtml" method="get">
        ...
        <table class="product-items">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>[#if orderItem.trade.order?has_content]${orderItem.trade.order.sn}[/#if]</td>
                    <td>${orderItem.trade.createDate}</td>
                    <!-- <td>${orderItem.trade.amount}</td> -->
                    <td>${orderItem.trade.settle}</td>
                    <td>
                        [#if orderItem.trade.suppliered??&&orderItem.trade.suppliered=="true"]
                        已结算
                        [#else]
                        未结算
                        [/#if]
                    </td>
                    <td>${orderItem.trade.supplierDate}</td>
                </tr>
                [/#list]
            </tbody>
        </table>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $("#export_ss").click(function(){
                ...
                $.ajax({
                    url:"${base}/b2c/member/supplier/order_settle_account_export.jhtml",
                    type:"get",
                    data:{
