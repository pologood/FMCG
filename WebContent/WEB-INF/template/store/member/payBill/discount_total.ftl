    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        ...
        <ol class="breadcrumb">
          ...
          <li><a href="${base}/store/member/payBill/discount_total.jhtml">数据统计</a></li>
          <li class="active">优惠买单统计</li>
        </ol>
      </section>
      <!-- Main content -->
          ...
          <div class="col-md-12">
            <div class="nav-tabs-custom">
              <ul class="nav nav-tabs pull-right">
                <li class="pull-left header"><i class="fa fa-pie-chart"></i>优惠买单统计</li>
              </ul>
              <div class="tab-content" style="padding:15px 0 0 0;">
                <form id="listForm" action="discount_total.jhtml" method="get">
                      ...
                      <div class="btn-group">
                        ...
                        <div class="dropdown fl ml5">
                          ...
                          <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu2" id="typeOption">
                            <li role="presentation" [#if type=="" ]class="active"[/#if]>
                              <a href="javascript:;" role="menuitem" val="">全部</a>
                            </li>
                            [#list types as types]
                            <li role="presentation" [#if types==type]class="active"[/#if]>
                              <a href="javascript:;" role="menuitem" val="${types}">${message("PayBill.Type."+types)}</a>
                            </li>
                            [/#list]
                          </ul>    
                        </div>
                    ...
                    <div class="col-sm-5">
                      <div class="box-tools fr">
                        <div class="input-group input-group-sm" style="width: 150px;">
                          <input type="text"id="keyword" name="keywords" value="${keywords}" maxlength="200" class="form-control pull-right" placeholder="流水号">
                    ...
                    <div class="box-body">
                      <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td>${payBill.createDate?string('yyyy-MM-dd')}</td>
                            <td>${(payBill.member.displayName)!}</td>
                            <td>${(payBill.deliveryCenter.name)!}</td>
                            <td>${(payBill.sn)!}</td>
                            <td>${(payBill.activityName)!}</td>
                            <td>${message("PayBill.Status."+payBill.status)}</td>
                            <td>${message("PayBill.Type."+payBill.type)}</td>
                            <td>${(payBill.clearingAmount)!}</td>
                            <td>${(payBill.amount)!}</td>
                            <td>${(payBill.backDiscount)!}</td>
                            <td>${(payBill.tenantDiscount)!}</td>
                            <td>${(payBill.discount)!}</td>
                          </tr>
                          [/#list]
                        </tbody>
                      </table>
  ...
  <script type="text/javascript">
    $().ready(function () {
      $("#export_ss").click(function () {
        ...
        $.ajax({
          url: "${base}/store/member/payBill/discount_total_export.jhtml",
          type: "get",
          data: {
