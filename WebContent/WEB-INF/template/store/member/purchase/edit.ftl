								...
								<div class="tab-content">
									<form class="form-horizontal" role="form">
										<div class="form-group">
											...
											<div class="col-sm-6">
												<input type="text" class="form-control" value="${purchase.sn}" readonly>
											</div>
											<div class="col-sm-2"><a href="${base}/store/member/purchase/print.jhtml?id=${purchase.id}" target="_blank"><button type="button" class="btn btn-block btn-default">打印</button></a></div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label">*要货时间：</label>
											<div class="col-sm-6">
												<input type="text" class="form-control" value="${(purchase.purchaseDate)!}" readonly>
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label">供应商：</label>
											<div class="col-sm-6">
												<input type="text" name="modifyDate" class="form-control" value="${(purchase.supplier.name)!}" readonly/>
											</div>
										</div>
										<!-- /.box -->
										<div class="row">
													...
													<div class="box-body">
														<table id="listTable" class="table table-bordered table-striped">
															...
															<tbody>
																...
																<tr>
																	<td title="${purchaseItem.name}">${abbreviate(purchaseItem.name,20,"...")}</td>
																	<td>${purchaseItem.sn}</td>
																	<td title="${purchase.supplier.name}">${abbreviate(purchase.supplier.name,16,"...")}</td>
	...
	<script type="text/javascript">
		$().ready(function () {
	  		...
	  		$("#confirm").click(function () {
	  			...
	  			[#list purchase.purchaseItems as purchaseItem]
	  			_productSn.push(${purchaseItem.sn});
	  			_quartitys.push(${purchaseItem.quantity});
	  			[/#list]

	  			$.ajax({
	  				url: "${base}/store/member/purchase/update/${purchase.id}.jhtml",
	  				data:{
	  					sns:_productSn,
