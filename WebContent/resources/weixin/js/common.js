...
$(function(){
	...
	if($('#m_shoptscroll').length > 0){
		var recommendlist = $('#m_shoptscroll ul').children('li').length;
		$('#m_shoptscroll ul').css('width',recommendlist * 80 + 'px');
		
		var pr_scroller = new iScroll('m_shoptscroll', {
	                snap: true,
	                momentum: true,
	         });
	}
	...
	if($('#m_scrooler_1').length > 0){
		var actscrooler1  = $('#m_scrooler_1'),
			actscrooll1 = new iScroll('m_scrooler_1',{onBeforeScrollStart:function(){actscrooler1.iScroll('refresh');}});
	}
	//article 分类滚动
	...
	if($('#m_areascrooler_1').length > 0){
		var areascrooler1  = $('#m_areascrooler_1'),
			areascrooll1 = new iScroll('m_areascrooler_1',{onBeforeScrollStart:function(){areascrooler1.iScroll('refresh');}}),
			areascrooler2  = $('#m_areascrooler_2'),
			areascrooll2 = new iScroll('m_areascrooler_2',{onBeforeScrollStart:function(){areascrooler2.iScroll('refresh');}}),
			areascrooler3  = $('#m_areascrooler_3'),
			areascrooll3 = new iScroll('m_areascrooler_3',{onBeforeScrollStart:function(){areascrooler3.iScroll('refresh');}});
	}
	if($('.m_area').length > 0){
		var areascrooler4  = $('#m_areascrooler_4'),
			areascrooll4 = new iScroll('m_areascrooler_4',{onBeforeScrollStart:function(){areascrooler4.iScroll('refresh');}});
	}
	...
	if($('.m_roombanner').length > 0){
        ...
        var slide = new TouchSlider({id:'m_roomslider',before:function(index){
	...
	if($('.m_inquiry').length > 0){
		...
		$('.m_areal ul li').on('tap',function(){
			$(this).parents('.m_areal').find('li').removeClass('m_down');
	...
	if ($('.w_satrlist').length > 0) {
	 ...
	 //订单列表切换
	if($('.order_screen').length>0){
		var i="";
		var listid="odr_scrooler_";
		
		$('.order_screen ul li').on('tap',function(){
			i=$(this).index();
			$('.order_screen ul li').removeClass('current_screen');
	...
	if($('.all_order').length > 0){
            var odrscrooler0  = $('#odr_scrooler_0'),
			odrscrool0 = new iScroll('odr_scrooler_0',{onBeforeScrollStart:function(){odrscrooler0.iScroll('refresh');}}),
			odrscrooler1  = $('#odr_scrooler_1'),
			odrscrool1 = new iScroll('odr_scrooler_1',{onBeforeScrollStart:function(){odrscrooler1.iScroll('refresh');}}),
			odrscrooler2  = $('#odr_scrooler_2'),
			odrscrool2 = new iScroll('odr_scrooler_2',{onBeforeScrollStart:function(){odrscrooler2.iScroll('refresh');}}),
			odrscrooler3  = $('#odr_scrooler_3'),
			odrscrool3 = new iScroll('odr_scrooler_3',{onBeforeScrollStart:function(){odrscrooler3.iScroll('refresh');}}),
			odrscrooler4  = $('#odr_scrooler_4'),
			odrscrool4 = new iScroll('odr_scrooler_4',{onBeforeScrollStart:function(){odrscrooler4.iScroll('refresh');}});
	...
	//收藏
	if($('.m_collect_tab li').length>0){
		tabli1=	$('.m_collect_tab li.m_down').index(".m_collect_tab li");
		if(tabli1>0){
			$('.m_article_sp').hide();
			$('.m_article_shop').show();
		}else{
		...
		$('.m_collect_tab li').on('tap',function(){
	...
	$.checkLogin = function() {
		var result = false;
		$.ajax({
			url : "/tiaohuo/common/check.jhtml",
			type : "GET",
			dataType : "json",
			cache : false,
			async : false,
	...
	if($('.m_tips').length > 0){
		function m_tipsfun(){
			$('.m_tips').show().addClass('ddin');
			$('.m_tipsbg').show().addClass('ddin');
			$.later(function(){
				$('.m_tips').removeClass('ddin').addClass('ddout');
				$('.m_tipsbg').removeClass('ddin').addClass('ddout');
			},2000, false);	
			$.later(function(){
				$('.m_tips').remove();
				$('.m_tipsbg').remove();
	...
	ptips =(function(){
		...
		if ($.isPlainObject(arguments[0])) {
			...
			pwtipsData.content = arguments[1];
		} else {
			return false;
		}
		tipsw ="";
		if(pwtipsData.type=='warn'){
			tipsw ='<div class=\"m_tips\"><i class=\"iconfont\">&#xe60c</i><span>' + pwtipsData.content + '</span></div><div class=\"m_tipsbg\"></div>';
		}else if(pwtipsData.type=='error'){
			tipsw ='<div class=\"m_tips m_error\"><i class=\"iconfont\">&#xe60b</i><span>' + pwtipsData.content + '</span></div><div class=\"m_tipsbg\"></div>';
		}else if(pwtipsData.type=='success'){
			tipsw ='<div class=\"m_tips m_success\"><i class=\"iconfont\">&#xe60d</i><span>' + pwtipsData.content + '</span></div><div class=\"m_tipsbg\"></div>';
		}else if(pwtipsData.type=='tips'){
			tipsw ='<div class=\"m_tips\"><i class=\"iconfont\">&#xe60e</i><span>' + pwtipsData.content + '</span></div><div class=\"m_tipsbg\"></div>';
		}
		if($('.m_tips').length > 0){
			$('.m_tips').remove();
		}
	...
	//分享那妞
		$('.m_shareclose').on('tap',function(){
	       $.later(function(){
	             $('.m_share1').hide();
	            $('.m_sharebg').hide();
	       }, 500, false);    
	    });
	    $('.m_sharebg').on('tap',function(){
	       $.later(function(){
	             $('.m_share1').hide();
	            $('.m_sharebg').hide();
	       }, 500, false);   
