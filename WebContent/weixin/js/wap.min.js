    ...
    SilderGallery.prototype = {
        show:function () {
            ...
            if(pImage == "true"){
                ...
                $(".pagenum").html(this.data.productImages.length);
            };
...
var weixin = {
    ...
    accuracy: 0, // 位置精度
    init: function () {
        $.getJSON(weixin.url + '?url=' + encodeURIComponent(location.href.split('#')[0]), function (res) {
            wx.config({
                beta: true,
                debug: false,
                ...
                wx.scanQRCode({
                    ...
                    success: function (res) {
                        ...
                        var tt=new tenant(function (data) {
                            if(data.length>0){
                                //跳转商品详情页(TODO 后期地址要改)
                                location.href = location.origin + '/debug/product_details.html?id=' + data[0].id;
                            }else{
                                toast.show("此商品没有上架");
                            }
                        });
...
var dropmenu = {
    init: function (navbar1, navbar2, navbar3) {
        ...
        $(".dropmenu #navbar .weui-navbar__item").click(function () {
    ...
    bindCity: function (data, fn) {
        var html = '';
        var chtml = '';
        html += '<ul class="menu-item weui-flex__item f14">';
        ...
        show:function () {
            ...
            for (var i = 0; i < this.data.length; i++) {
                if (tNum==0){
                    ...
                    str += '<p><img src="' + this.data[i].image + '"></p>';
                    str += '<p class="Ftext">' + this.data[i].name + '</p>';
                    str += '</a></div>';
                    if (((i%rNum)==iNum) && (i>0)) {
                    ...
                    str += '<div class="channel__item"><a href="product_details.html?id='+ this.data[i].id +'" data-id="'+ this.data[i].id +'"> ';
                    str += '<p><img src="' + this.data[i].thumbnail + '"></p>';
                    str += '<p class="f14">' +
                        '<span>' + this.data[i].price + '<i class="f12">' + this.data[i].marketPrice + '</i></span>' +
            ...
            success: function (data) {
                ...
                $.each(data, function (index, bank) {
                    ...
                    div+= '<div class="weui-cell" id="' + bank.id + '" account="' + bank.account + '" name="' + bank.bankName + '" cardNo="' + bank.cardNo + '" >'+
                        ...
                        '<img src="images/static/cloud_control.png" alt="" >'+
                        '</div>'+
                        ...
                        '<p class="seletecard"><span><i class="icon iconfont icon-delete-01"></i>删除</span></p>'+
                        '</div>'+
                        '</div>'+
                        '</div>';
                    html+=div;
...
//             if (starnum_int <= 6) {
//                 $ele.children().eq(starnum_int).addClass("icon-s-star");
//             }
//             for (var i = starnum_int ; i < 5; i++) {
//                 $ele.children().eq(i).addClass("icon-l-star");
//             }
//             //set flag
    ...
    bank: function (data) {
        ajax.get({
            ...
            success: function (data) {
                var html = "";
                $.each(data, function (index, bank) {
                    var ul = '<a class="weui-cell weui-cell_access" href="javascript:;" adid="' + bank.id + '">';
                    ul +='<div class="weui-cell__bd">'+
                        '<p class="f14">'+
                        '收货人：<span class="orange">'+bank.consignee+'</span>,&nbsp;<span class="orange">'+bank.phone+'</span>'+
                        '</p>'+
                        '<p class="f12">'+
                        '<i class="icon iconfont icon-s-location"></i><span>'+bank.area.fullName+'</span>,<span>'+bank.address+'</span>'+
                        '</p>'+
                        '</div>'
                    ul += '</a>';
                    html += ul;
