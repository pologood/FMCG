 ...
 * @version 3.0
 */
@Controller("adminCsiipayController")
@RequestMapping("/admin/payment_plugin/csiipay")
public class CsiipayController extends BaseController {

	@Resource(name = "csiipayPlugin")
	private CsiipayPlugin csiipayPlugin;
	...
	public @ResponseBody
	Message install() {
		if (!csiipayPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(csiipayPlugin.getId());
	...
	Message uninstall() {
		if (csiipayPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = csiipayPlugin.getPluginConfig();
