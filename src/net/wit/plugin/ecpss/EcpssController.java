 ...
 * @version 3.0
 */
@Controller("adminEcpssController")
@RequestMapping("/admin/payment_plugin/ecpss")
public class EcpssController extends BaseController {

	@Resource(name = "ecpssPlugin")
	private EcpssPlugin ecpssPlugin;
	...
	public @ResponseBody
	Message install() {
		if (!ecpssPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(ecpssPlugin.getId());
	...
	Message uninstall() {
		if (ecpssPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = ecpssPlugin.getPluginConfig();
	...
	public String setting(ModelMap model) {
		...
		return "/net/wit/plugin/ecpss/setting";
	}
