 ...
 */
@Controller("posConsumerController")
@RequestMapping("/pos/consumer")
public class ConsumerController extends BaseController {
	...
	@ResponseBody
	public DataBlock sendMobile(Long tenantId,String key,String mobile,HttpServletRequest request) {
	       Tenant tenant = tenantService.find(tenantId);
	        if (tenant==null) {
	        	return DataBlock.error(DataBlock.TENANT_INVAILD);
	        }
			ResourceBundle bundle=PropertyResourceBundle.getBundle("config");
			String myKey = DigestUtils.md5Hex(mobile+tenantId.toString()+bundle.getString("appKey"));
			if (!myKey.equals(key)) {
	...
	@ResponseBody
	public DataBlock checkCaptcha(Long tenantId,String key,String captcha,HttpServletRequest request, HttpServletResponse response) {
	       Tenant tenant = tenantService.find(tenantId);
	        if (tenant==null) {
	        	return DataBlock.error(DataBlock.TENANT_INVAILD);
	        }
			ResourceBundle bundle=PropertyResourceBundle.getBundle("config");
			String myKey = DigestUtils.md5Hex(captcha+tenantId.toString()+bundle.getString("appKey"));
			if (!myKey.equals(key)) {
				return DataBlock.error("通讯密码无效");
			}
		HttpSession session = request.getSession();
	...
	@ResponseBody
	public DataBlock add(Long tenantId,String key,String mobile,Long areaId,String captcha, HttpServletRequest request, HttpServletResponse response) {
	       Tenant tenant = tenantService.find(tenantId);
	        if (tenant==null) {
	        	return DataBlock.error(DataBlock.TENANT_INVAILD);
	        }
			ResourceBundle bundle=PropertyResourceBundle.getBundle("config");
			String myKey = DigestUtils.md5Hex(mobile+captcha+tenantId.toString()+bundle.getString("appKey"));
			if (!myKey.equals(key)) {
				return DataBlock.error("通讯密码无效");
			}
		HttpSession session = request.getSession();
	...
	@ResponseBody
	public DataBlock view(Long tenantId,String key,String mobile,HttpServletRequest request, HttpServletResponse response) {
	       Tenant tenant = tenantService.find(tenantId);
	        if (tenant==null) {
	        	return DataBlock.error(DataBlock.TENANT_INVAILD);
	        }
		    ...
		    List<Consumer> consumers = consumerService.findList(1, filters, null);
	...
	@ResponseBody
	public DataBlock view(Long tenantId,String key,Pageable pageable,HttpServletRequest request, HttpServletResponse response) {
	       Tenant tenant = tenantService.find(tenantId);
	        if (tenant==null) {
	        	return DataBlock.error(DataBlock.TENANT_INVAILD);
	        }
			...
			List<Filter> filters = new ArrayList<Filter>();
			filters.add(new Filter("tenant", Operator.eq, tenant));
			filters.add(new Filter("status", Operator.eq, Consumer.Status.enable));
			pageable.setFilters(filters);
		    Page<Consumer> consumers = consumerService.findPage(pageable);
        return DataBlock.success(MemberModel.bindConsumerData(consumers.getContent()),"执行成功");
	}
