 ...
 */
@Controller("appMemberCashController")
@RequestMapping("/app/member/cash")
public class CashController extends BaseController {
	 ...
	 * @param amount 提现金额
	 * @param password 加密后的支付密码 password 不需要传
	 */
	@RequestMapping(value = "/save", method = RequestMethod.POST)
	@ResponseBody
	public DataBlock submit(Long memberBankId,BigDecimal amount,String password,String type, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		try {
			...
			if(member.getTenant()!=null){
				if(credit.getStatus() == Credit.Status.success||credit.getStatus() == Credit.Status.wait){
	...
	@ResponseBody
	public DataBlock calculate(Method method, BigDecimal amount, HttpServletRequest request) {
		Member member = memberService.getCurrent();
		if (member==null) {
			DataBlock.error(DataBlock.SESSION_INVAILD);
		}
