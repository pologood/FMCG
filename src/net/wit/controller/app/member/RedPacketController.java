 ...
 */
@Controller("appMemberRedPacketController")
@RequestMapping("/app/member/redPacket")
...
//        couponService.refreshStatus(tenant);
    ...
    @ResponseBody
    public DataBlock add(BigDecimal amount, Integer count, BigDecimal minimumPrice, BigDecimal freezeAmount) {
        Member member = memberService.getCurrent();
        if (member == null) {
            return DataBlock.error(DataBlock.SESSION_INVAILD);
        }
    ...
    public DataBlock viewList(Long id,String type, Pageable pageable) {
        ...
        return DataBlock.success(RedPacketListModel.bindData(couponCodes.getContent()), "执行成功");
    }
    ...
    public DataBlock extensionBouns() {
        ...
        map.put("verificationCount", rebateService.count(member, Rebate.Type.sale));
    ...
    public DataBlock extensionBouns(String type,Pageable pageable) {
        ...
        if(type.equals("0")){//推广
            Page<CouponCode> couponCodes = couponCodeService.findCoupon(member,type,pageable);
            ...
            Page<Rebate> page=rebateService.findPage(member,Rebate.Type.sale,null,pageable);
            redPacketListModels = RedPacketListModel.bindRebate(page.getContent());
        }

        return DataBlock.success(redPacketListModels, "执行成功");
    }
