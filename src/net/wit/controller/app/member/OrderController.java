...
public class OrderController extends BaseController {
     ...
     * 计算(微信使用)
     */
    @RequestMapping(value = "/calculateW", method = RequestMethod.POST)
    public
    @ResponseBody
    DataBlock calculateW(Long paymentMethodId, Long shippingMethodId, Long[] tenantIds, String[] code, Boolean isInvoice, String invoiceTitle, Boolean useBalance, String[] memo) {
        Map<String, Object> data = new HashMap<String, Object>();
        Cart cart = cartService.getCurrent();
        Member member = memberService.getCurrent();
        if (member == null) {
    ...
    DataBlock create(Long receiverId, Long paymentMethodId, Long shippingMethodId, String[] code, Boolean isInvoice,
                     String invoiceTitle, Boolean useBalance, Long[] tenantIds, String[] memo, Long[] deliveryCenterIds, Long extensionId, String uuid, String token_key, HttpServletRequest request) {
        if (extensionId != null) {
        ...
        Order order = orderService.create(cart, receiver, paymentMethod, shippingMethod, couponCodes, isInvoice, invoiceTitle, useBalance, tenantIds, memo, deliveryCenters, member, null, OrderSource.app, equipment, token_key);
