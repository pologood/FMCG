...
import net.wit.controller.store.BaseController;
 ...
 * @version 3.0
 */
@Controller("storeMemberMessageController")
@RequestMapping("/store/member/message")
public class MessageController extends BaseController {
	...
	public String send(Long draftMessageId, Model model) {
		...
		return "/store/member/message/send";
	}
	...
	public String send(Long draftMessageId, String username, String title, String content, @RequestParam(defaultValue = "false") Boolean isDraft, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		if (isDraft) {
			addFlashMessage(redirectAttributes, net.wit.Message.success("shop.member.message.saveDraftSuccess"));
			return "redirect:draft.jhtml";
		} else {
			addFlashMessage(redirectAttributes, net.wit.Message.success("shop.member.message.sendSuccess"));
	...
	public String view(Long id, Model model) {
		...
		return "/store/member/message/view";
	}
	...
	public String reply(Long id, String content, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		if ((member.equals(forMessage.getReceiver()) && !forMessage.getSenderDelete()) || (member.equals(forMessage.getSender()) && !forMessage.getReceiverDelete())) {
			...
			addFlashMessage(redirectAttributes, net.wit.Message.success("shop.member.message.replySuccess"));
			return "redirect:list.jhtml";
		}
	}
	...
	public String list(Pageable pageable, Model model) {
		...
		Member member = memberService.getCurrent();
		model.addAttribute("member", member);
		model.addAttribute("page", messageService.findReceivePage(member, pageable));
		model.addAttribute("menu","message");
		model.addAttribute("area",areaService.getCurrent());
		return "/store/member/message/list";
	}
	...
	public String draft(Integer pageNumber, Model model) {
		...
		return "/store/member/message/draft";
	}
	...
	public @ResponseBody
	net.wit.Message delete(Long id) {
		Member member = memberService.getCurrent();
	...
	public @ResponseBody
	net.wit.Message deleteAll() {
		Member member = memberService.getCurrent();
		List<Message> messages= messageService.findList(member, null, null, null);
		for(Message message:messages){
			messageService.delete(message.getId(), member);
		}
		return net.wit.Message.success("操作完成");
	}
	...
	public @ResponseBody
	net.wit.Message read(Long id) {
		Member member = memberService.getCurrent();
		Message message = messageService.find(id);
		if (message == null || message.getIsDraft() || message.getForMessage() != null) {
			return net.wit.Message.error("找不到该信息");
		}
		if(member==message.getReceiver()){
			message.setReceiverRead(true);
		}else if(member==message.getSender()){
			message.setSenderRead(true);
		}
		messageService.update(message);
		return net.wit.Message.success("操作成功");
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/read_all", method = RequestMethod.POST)
	public @ResponseBody
	net.wit.Message readAll() {
		Member member = memberService.getCurrent();
		List<Message> messages= messageService.findList(member, null, null, null);
		for(Message message:messages){
			if (message == null || message.getIsDraft() || message.getForMessage() != null) {
