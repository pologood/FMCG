 ...
 */
@Controller("storeMemberStatisticsDateController")
@RequestMapping("/store/member/statistics")
public class StatisticsDataController extends BaseController {
    ...
    public String saleTotal(String type,String begin_date, String end_date,Long couponId, String keywords, Model model, Pageable pageable) {
        ...
        return "/store/member/statistics/sale_total";
    }
    ...
    public List<Map<String, Object>> saleTotalExport( String begin_date, String end_date, String keywords) {
        ...
        for(Trade trade:trades){
        	...
        	map.put("time", sdf.format(trade.getCreateDate()));
        	map.put("sn", trade.getSn());
    ...
    public String saleDetail( String begin_date, String end_date, String keywords, Model model, Pageable pageable) {
        ...
        return "/store/member/statistics/sale_detail";
    }
    /**
     * 销售明细（聚德汇-导出功能）
     */
    ...
    public List<Map<String, Object>> saleDetailExport(Long sellerId, String begin_date, String end_date, String keywords) {
        ...
        for(OrderItem orderItem:orderItems){
			Map<String, Object> map = new HashMap<String,Object>();
			map.put("create_date", sdf.format(orderItem.getCreateDate()));
			if(orderItem.getTrade().getShippingDate()!=null){
				map.put("shipping_date", sdf.format(orderItem.getTrade().getShippingDate()));
			}else{
			...
			map.put("nickname", orderItem.getOrder().getConsignee());
			...
			map.put("totalPrice", new BigDecimal(orderItem.getQuantity()).multiply(orderItem.getPrice()));
			map.put("cost", orderItem.getCost());
			map.put("totalCost", new BigDecimal(orderItem.getQuantity()).multiply(orderItem.getCost()));
			...
			if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.paid){
				map.put("paymentStatus","已支付");
			}else if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.unpaid){
				map.put("paymentStatus","未支付");
			}else if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.partialPayment){
				map.put("paymentStatus","部分支付");
			}else if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.partialRefunds){
				map.put("paymentStatus","部分退款");
			}else if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.refunded){
				map.put("paymentStatus","已退款");
			}else if(orderItem.getTrade().getPaymentStatus()== Order.PaymentStatus.refundApply){
	...
	public String productTotal(String type,String begin_date, String end_date,Long couponId, String keywords, Model model, Pageable pageable) {
		...
		return "/store/member/statistics/product_total";
	}
	/**
	 * 产品统计（聚德汇-导出）
	 */
    ...
    public String returnDetail(String page_numb, Long sellerId, String begin_date, String end_date, String keywords, Model model, Pageable pageable) {
        ...
        Page<SpReturnsItem> page=spReturnsItemService.findPageByTenant(tenant, null, beginDate, endDate, null, null, keywords, pageable);
        model.addAttribute("page", page);
        model.addAttribute("begin_date", begin_date);
        model.addAttribute("end_date", end_date);
        model.addAttribute("keywords", keywords);
        model.addAttribute("menu", "return_detail");
        return "/store/member/statistics/return_detail";
    }
    /**
     * 退货明细（聚德汇-导出功能）
     */
    ...
    public List<Map<String, Object>> returnDetailExport(Long sellerId, String begin_date, String end_date, String keywords) {
        ...
        List<Map<String, Object>> maps=new ArrayList<Map<String, Object>>();
        List<SpReturnsItem> spReturns=spReturnsItemService.findByTenant(tenant, null, beginDate, endDate, null, keywords);

		for(SpReturnsItem returnItem:spReturns){
			Map<String, Object> map=new HashMap<String,Object>();
			map.put("time", sdf.format(returnItem.getCreateDate()));
	...
	public String returnTotal(String page_numb, Long sellerId, String begin_date, String end_date, String keywords, Model model, Pageable pageable) {
		...
		return "/store/member/statistics/return_total";
	}
	/**
	 * 退货统计（聚德汇-导出功能）
	 */
	...
	public List<Map<String, Object>> returnTotalExport(Long sellerId, String begin_date, String end_date, String keywords) {
		...
		for(SpReturns spReturn:spReturns){
			...
			if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.paid){
				map.put("paymentStatus","已支付");
			}else if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.unpaid){
				map.put("paymentStatus","未支付");
			}else if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.partialPayment){
				map.put("paymentStatus","部分支付");
			}else if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.partialRefunds){
				map.put("paymentStatus","部分退款");
			}else if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.refunded){
				map.put("paymentStatus","已退款");
			}else if(spReturn.getTrade().getPaymentStatus()== Order.PaymentStatus.refundApply){
				map.put("paymentStatus","退款中");
			}
	...
	public String returnProductTotal(String type,String begin_date, String end_date,Long couponId, String keywords, Model model, Pageable pageable) {
		...
		return "/store/member/statistics/return_product_total";
	}
	/**
	 * 退货产品统计（聚德汇-导出）
	 */
