 ...
 * @version 3.0
 */
@Controller("assistantCmbcPayController")
@RequestMapping("assistant/cmbcpay")
public class CmbcPayController extends BaseController {
	...
	public DataBlock submit(Long tenantId,String key,String type,String safeKey,BigDecimal amount,Long unionTenantId ,HttpServletRequest request) {
		...
		if (type.equals("1")) {
			...
			data.put("platformId", "A00002016120000000294");
			data.put("merchantNo", "M01002016090000001273");
			data.put("selectTradeType", "API_WXQRCODE");
		} else {
		...
		data.put("transTime", "20170118102701172");
		data.put("transDate", "20170118");
		data.put("merchantSeq", "A00002016120000000294T102701172");
		data.put("orderInfo", "统一下单API测试-API_WXQRCODE");
		data.put("amount",payment.getAmount().toString());
		
		String context = JSONObject.toJSONString(data);
		//签名
		String sign = SignEncryptDncryptSignChk.getSign(context);
		//加密前
		String signContext = SignEncryptDncryptSignChk.sign(sign, context);
		//加密后
		String params = SignEncryptDncryptSignChk.encrypt(signContext);


			String cmbcpay = bundle.getString("cmbcpay");
			String rtn_msg = HttpClientUtil.doPost(cmbcpay,params);
			if("".equals(rtn_msg)){
