...
public class IndexController extends BaseController {
	...
	public @ResponseBody Map<String, Object> build(BuildType buildType, Boolean isPurge, Integer first, Integer count) {
		...
		} else if (buildType == BuildType.deliveryCenter) {
			if (first == 0 && isPurge != null && isPurge) {
				searchService.purge(DeliveryCenter.class);
			}
			List<DeliveryCenter> deliverys = deliveryCenterService.findList(null, null, null, first, count);
			for (DeliveryCenter delivery : deliverys) {
				searchService.index(delivery);
				buildCount++;
			}
			first += deliverys.size();
			if (deliverys.size() == count) {
				isCompleted = false;
			}
		}
