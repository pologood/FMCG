 ...
 */
@Controller("wapMemberPayBillController")
@RequestMapping("/wap/member/pay/bill")
public class PayBillController extends BaseController {
    ...
    DataBlock getAmount(Long id, BigDecimal amount, @RequestParam(defaultValue = "0")BigDecimal noAmount, Long deliveryCenterId,HttpServletRequest request) {
        ...
        BigDecimal tenantDiscount = BigDecimal.ZERO;    //店铺优惠金额
        BigDecimal backDiscount = BigDecimal.ZERO;      //店铺返现金额

        BigDecimal discount = BigDecimal.ZERO;          //平台活动优惠金额
        ...
        if(tenant.getIsUnion()){
            //activityCoupon = couponService.find(1L);
            //List<CouponCode> couponCodes=couponCodeService.findCoupon(member,activityCoupon);
