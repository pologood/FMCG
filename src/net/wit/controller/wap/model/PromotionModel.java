...
public class PromotionModel extends BaseModel {
	...
	public static Set<PromotionModel> bindData(Set<Promotion> promotions) {
		Set<PromotionModel> models = new HashSet<PromotionModel>(promotions.size());
	...
	public static List<PromotionModel> bindData(List<Promotion> promotions) {
		...
		for (Promotion promotion:promotions) {
			...
			if(promotion.getType()==Type.buyfree||promotion.getType()==Type.seckill){
				PromotionModel model = new PromotionModel();
				model.copyFrom1(promotion);
				models.add(model);
			}
	...
	public void copyFrom1(Promotion promotion) {
		//this.id = promotion.getId();
		this.name = promotion.getName();
		this.title = promotion.getTitle();
		if (this.title==null) {
