...
public class ArticleServiceImpl extends BaseServiceImpl<Article, Long> implements ArticleService, DisposableBean {
	...
	public List<Article> findList(Set<ArticleCategory> articleCategories, List<Tag> tags, Tenant tenant, Area area, Integer count, List<Filter> filters, List<Order> orders) {
		return articleDao.findList(articleCategories, tags, tenant, area, count, filters, orders);
	}

	@Transactional(readOnly = true)
	@Cacheable("article")
	public List<Article> findList(Set<ArticleCategory> articleCategories, List<Tag> tags, Tenant tenant, Area area, Integer count, List<Filter> filters, List<Order> orders, String cacheRegion) {
		return articleDao.findList(articleCategories, tags, tenant, area, count, filters, orders);
	}
