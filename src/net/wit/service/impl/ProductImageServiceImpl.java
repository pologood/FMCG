...
public class ProductImageServiceImpl implements ProductImageService, ServletContextAware {
				...
				public void run() {
					...
					for (StoragePlugin storagePlugin : storagePlugins) {
						if (storagePlugin.getIsEnabled()) {
							...
							System.out.println(setting.getWatermarkImage().getBytes()+" ------>  "+Base64.encodeBase64String(setting.getWatermarkImage().getBytes()));
							...
							try {
								try{
									if (storagePlugin.getPluginConfig().getAttribute("isCorpEnabled").equals("true"))
									{
	...
	public void build(ProductImage productImage) {
		...
		File localFile = productImage.getLocalFile();
		if (multipartFile != null && !multipartFile.isEmpty()) {
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						try{
								if (storagePlugin.getPluginConfig().getAttribute("isCorpEnabled")!=null)
								{
									if (storagePlugin.getPluginConfig().getAttribute("isCorpEnabled").equals("true"))
									{
										productImage.setSource(storagePlugin.getUrl(sourcePath));
										productImage.setLarge(storagePlugin.getUrl(sourcePath)+"@"+setting.getLargeProductImageWidth()+"w_"+setting.getLargeProductImageHeight()+"h_1e_1c_100Q");
										productImage.setMedium(storagePlugin.getUrl(sourcePath)+"@"+setting.getMediumProductImageWidth()+"w_"+setting.getMediumProductImageHeight()+"h_1e_1c_100Q");
										productImage.setThumbnail(storagePlugin.getUrl(sourcePath)+"@"+setting.getThumbnailProductImageWidth()+"w_"+setting.getThumbnailProductImageHeight()+"h_1e_1c_100Q");
									}
								}
								else{
									productImage.setSource(storagePlugin.getUrl(sourcePath));
			...
			File tempFile = new File(System.getProperty("java.io.tmpdir") + "/upload_" + UUID.randomUUID() + ".tmp");
			FileUtil.copyFile(localFile, tempFile);
			try {
				Setting setting = SettingUtils.get();
				Map<String, Object> model = new HashMap<String, Object>();
				model.put("uuid", UUID.randomUUID().toString());
				String uploadPath = FreemarkerUtils.process(setting.getImageUploadPath(), model);
				String uuid = UUID.randomUUID().toString();
				String sourcePath = uploadPath + uuid + "-source." + FilenameUtils.getExtension(localFile.getName());
				String largePath = uploadPath + uuid + "-large." + DEST_EXTENSION;
				String mediumPath = uploadPath + uuid + "-medium." + DEST_EXTENSION;
				String thumbnailPath = uploadPath + uuid + "-thumbnail." + DEST_EXTENSION;
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						addTask(sourcePath, largePath, mediumPath, thumbnailPath, tempFile, localFile.getName());
						try{
							if (storagePlugin.getPluginConfig().getAttribute("isCorpEnabled")!=null)
							{
								if (storagePlugin.getPluginConfig().getAttribute("isCorpEnabled").equals("true"))
	...
	public String  waterimage(){
		...
		try
		{
			String baseString4 = Base64.encodeBase64String(setting.getWatermarkImage().getBytes());
				
			if (!setting.getWatermarkPosition().equals(Setting.WatermarkPosition.no))
			{
				if (setting.getWatermarkPosition().equals(Setting.WatermarkPosition.topLeft))
				{
					
					return "@watermark=1&&object="+baseString4+"&t="+setting.getWatermarkAlpha() +"&p=1";
				}
